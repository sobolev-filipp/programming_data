...
**Модуль 2 · Фотомагия в Photoshop · Урок 1 из 15 | 2 ак.ч. (1,5 часа)**
...
**Задача:** взять фотографию улицы и фотографию кота — и «поселить» **огромного кота посреди города**, будто он ростом с дом. Кот будет стоять на асфальте, отбрасывать тень — как настоящий гигантский зверь из фильма-катастрофы, только смешной.

**В итоге у тебя получится:** одна картинка-коллаж, собранная из **трёх слоёв** (город → тень → кот), где всё выглядит цельно. Ты сохранишь её в двух видах: рабочий файл `.psd` (чтобы можно было доработать) и готовый `.png` (чтобы показать друзьям).

**Чему это научит:** понять **слои** — главную идею Photoshop, из которой вырастет весь курс. Кот сегодня — только повод; тот же приём потом соберёт афишу фильма и обложку.

> ⚠️ Всё делаем **вручную** — никакого «умного» автоподбора. Так ты по-настоящему поймёшь, как устроена программа.
...
| 🪄 | **Быстрое выделение** (`W`) | обвести объект, чтобы вырезать |
...
> 💡 **Фишка про маленький треугольник.** У некоторых иконок в углу — **маленький треугольник ◣**. Это значит, что там **спрятана группа инструментов**. Нажми и подержи иконку — выпадет список. Например, под «Быстрым выделением» прячется «Волшебная палочка» (выделяет по цвету).
...
1. В **панели слоёв** (справа снизу) кликни по слою «кот». Создай **новый пустой слой**: внизу этой панели нажми иконку **листа с загнутым уголком** (наведи — подсказка «Создать новый слой»). Затем **мышкой перетащи** этот новый слой **вниз, ПОД слой «кот»** (тень должна быть за котом, но над городом). Двойной клик по имени слоя → переименуй в «тень».
2. Возьми **«Кисть»**: на панели инструментов **слева** — иконка кисточки (или клавиша `B`). Вверху, **под строкой меню**, появится панель параметров кисти — поставь: **Жёсткость 0%** (кликни на треугольник ▾ у образца кисти → ползунок «Жёсткость»), **Размер** 150–250 пикс (там же, или клавиши `[` меньше / `]` больше). **Цвет чёрный** — нажми `D` (сбросит цвета внизу панели инструментов на чёрно-белые).
3. Проведи кистью по асфальту **у лап кота**, в ту сторону, куда падает свет — получится тёмное пятно-«лужа».
4. Вверху **панели слоёв** есть поле **«Непрозр.»** (100%) — кликни по числу и поставь **~40–60%**: настоящая тень не бывает угольно-чёрной.
...
> 💡 **Фишка «убрать лишнее вручную».** Если на фоне торчит мелкий отвлекающий предмет (провод, мусорка), его убирают инструментом **«Штамп»** (`S`): зажми `Alt` и кликни по «чистому» кусочку асфальта рядом (взял образец), потом закрась им лишний предмет. Инструмент **копирует** один участок картинки на другой. Подробно поработаем со «Штампом» на уроке ретуши — сегодня просто знай, что так можно.
...
Если у тебя не PNG, а обычное фото кота на фоне — отделим кота от его фона **вручную**.

1. Встань на слой с фото кота. Возьми инструмент **«Быстрое выделение»** (`W`) — это кисть, которая «прилипает» к краям объекта.
2. **Проводи** кистью по коту (не по фону) — выделение (пунктир-«муравьи») будет расти, само останавливаясь у краёв шерсти. Захватило лишний фон? Зажми **`Alt`** и проведи по лишнему — уберётся. Размер кисти — клавиши `[` и `]`.
3. Когда кот обведён, нажми сверху **«Выделение и маска…»**. Откроется отдельная комната-редактор:
   - слева возьми **«Уточнить края»** (кисточка) и проведи по краю **шерсти** — программа аккуратно вытянет пушистые края (это не ИИ, а анализ контраста);
   - справа в «Вывод» выбери **«Новый слой со слоем-маской»** → **ОК**.
4. Готово: кот теперь на **прозрачном фоне** (рядом со слоем появится чёрно-белая **маска**). Ставь его в коллаж как обычно.
...
## ✅ Как правильно / ❌ Как НЕ надо (коллаж)

| ❌ Как НЕ надо | ✅ Как правильно |
|----------------|------------------|
| Объект «наклеен»: без тени, случайный размер | **Тень у ног** + масштаб по окружению (машины/люди рядом) |
| Всё намешано на одном слое, стёр ластиком | Каждый объект — **свой слой**, порядок в панели слоёв |
| Свет объекта спорит с фоном | **Один источник света:** тени смотрят в одну сторону |

> ⚠️ Главная ошибка новичка — **забыть тень**. Без «контактной» тени даже точный монтаж выглядит наклейкой.
