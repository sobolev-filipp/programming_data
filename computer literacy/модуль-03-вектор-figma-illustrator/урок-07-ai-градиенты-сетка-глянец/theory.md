...
Узел сетки — это маленький **ромбик** на пересечении линий. Красим так: **выдели узел → задай ему цвет**.

1. Возьми **«Прямое выделение»** (`A`, белая стрелка) → кликни **узел в правом-нижнем** углу шара (ромбик подсветится). Теперь задай цвет: двойной клик по **«Заливке»** (внизу панели инструментов) → `#` **C8741A** (тёмно-оранжевый) → ОК. 👀 Этот угол потемнел — сторона **в тени**.
2. Кликни (`A`) **узел вверху-слева** → «Заливка» **#FFD98A** (светло-оранжевый) — освещённый бок.
3. **Блик:** кликни самый верх-левый узел → «Заливка» почти белый **#FFF3D6**.
4. *(нужен ещё узел?)* Возьми инструмент **«Сетка»** (`U`) и **кликни** по фигуре — добавится новый узел там, где нужен переход.

**👀 Что произойдёт:** апельсин стал **объёмным** — свет плавно перетекает в тень. Это магия сетки: цвет «прилипает» к узлу и мягко растекается вокруг.

> ⚠️ **Ошибка:** кликнул «Прямым выделением» **мимо** узла (по пустому месту сетки) — тогда покрасится не тот кусок. Целься точно в **ромбик** узла; когда он выделен — он крупнее.
