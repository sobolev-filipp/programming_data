...
- **🅰️ Фигура точного размера:** возьми инструмент (напр. «Эллипс» `L`) и **один клик** по холсту (не тяни!) → откроется окно с полями **Ширина/Высота** → впиши числа → ОК. Появится ровно такой размер. *(Так задаём все размеры в уроке — «круг 180×180» значит: клик → 180 и 180.)*
- **🎨 Заливка и обводка** — **внизу панели инструментов** два квадрата (верхний — Заливка, за ним — Обводка). **Двойной клик по «Заливке»** → палитра, внизу поле **`#`** → впиши код цвета (напр. `F6A5C0`) → ОК. **Убрать обводку:** кликни квадрат «Обводка» → значок **«Нет»** (⃠) внизу, или клавиша `/`.
...
**👀 Что произойдёт:** от кругов останется **половина = полукруг** — ломтик арбуза. 🍉
