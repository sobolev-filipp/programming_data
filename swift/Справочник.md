...
if let g = motion.deviceMotion?.gravity {
    hero.position.x += CGFloat(g.x) * 420 * dt   // -1 (влево) … +1 (вправо)
}
// когда сцена уходит:
...
// следовать за героем (в update):
cam.position.y = hero.position.y + size.height * 0.20
```
...
override func update(_ currentTime: TimeInterval) {
    ...
    hero.position.y += speedPerSec * dt         // путь = скорость × время
}
```
...
#### Столкновение по расстоянию (без физики)

```swift
let dx = a.position.x - hero.position.x
let dy = a.position.y - hero.position.y
if hypot(dx, dy) < rA + rHero {   // расстояние < суммы радиусов → задели
    crash()
}
```

> `hypot(dx, dy)` = √(dx²+dy²) — расстояние между центрами. Радиус круга можно взять из рамки: `node.frame.size.width / 2`. Лёгкая замена физике (урок 15), когда нужен только факт касания.

#### Спавн впереди и уборка позади

```swift
// спавн, пока «граница» ниже верха видимой зоны:
while nextY < cam.position.y + size.height {
    spawn(atY: nextY); nextY += CGFloat.random(in: 220...340)
}
// уборка ушедших за низ (иначе копятся и тормозят):
