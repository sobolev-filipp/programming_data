...
- [Блок 4 — Разработка игр SpriteKit](#блок-4--разработка-игр-spritekit)
  ...
  - [Урок 20 — Камера, параллакс, наклон, игровой цикл](#урок-20--камера-параллакс-наклон-игровой-цикл)
- [Блок 5 — SwiftUI](#блок-5--swiftui)
  - [Урок 28 — SwiftUI: первый взгляд](#урок-28--swiftui-первый-взгляд)
  - [Урок 29 — SwiftUI: списки и данные](#урок-29--swiftui-списки-и-данные)
  - [Урок 30 — SwiftUI: загрузка данных из сети](#урок-30--swiftui-загрузка-данных-из-сети)
- [Шпаргалки](#шпаргалки)
...
#### Мультитач — несколько касаний сразу

`touches.first` берёт только один палец. Чтобы обрабатывать все (например, двигаться и стрелять одновременно) — перебираем весь набор:

```swift
override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
    for touch in touches {                       // каждый палец отдельно
        let location = touch.location(in: self)
        // ... что делать с этим касанием
    }
}
```

#### Не выпускать за экран — clamp

Чтобы корабль не уехал за край, «зажимаем» координату между минимумом и максимумом:

```swift
let half = ship.size.width / 2
// max(нижняя граница, min(верхняя граница, значение))
let clampedX = max(half, min(size.width - half, targetX))
ship.position.x = clampedX
```

`max(a, min(b, x))` — читается «не меньше a и не больше b». Классический приём удержания объекта в рамках экрана.
...
#### Разные типы врагов — enum и switch

`enum` — список именованных вариантов (вместо чисел 0, 1, 2). `switch` — выбор «что делать для каждого варианта».
...
// Выбор типа с разной вероятностью (60% / 25% / 15%)
let roll = Int.random(in: 0...100)
let type: EnemyType = roll < 60 ? .normal : (roll < 85 ? .fast : .tank)

// Параметры под каждый тип
switch type {
case .normal: hp = 1; points = 10
case .fast:   hp = 1; points = 20
case .tank:   hp = 2; points = 30
}
```

#### userData — «карман» данных внутри объекта

У каждого узла есть словарь `userData`, куда можно спрятать свои данные (HP, очки конкретного врага). Так каждый враг помнит своё здоровье сам.

```swift
// Записать (словарь создать ОДИН раз, до записи!)
enemy.userData = NSMutableDictionary()
enemy.userData?["hp"] = 2
enemy.userData?["points"] = 30

// Прочитать: «взять как Int, а если пусто — подставить 1»
var hp = enemy.userData?["hp"] as? Int ?? 1
hp -= 1
enemy.userData?["hp"] = hp
```

`as? Int` — попробовать прочитать как число, `?? 1` — запасное значение, если пусто (защита от краша).

#### Прицеливание — atan2 и rotate(toAngle:)

`atan2(dy, dx)` возвращает угол направления (в радианах) от объекта к цели. `rotate(toAngle:)` поворачивает **в** конкретный угол.

```swift
let dx = target.x - cannon.position.x
let dy = target.y - cannon.position.y
let angle = atan2(dy, dx) - .pi / 2   // -90°, если ствол нарисован «вверх»
cannon.run(SKAction.rotate(toAngle: angle, duration: 0.1, shortestUnitArc: true))
```

`shortestUnitArc: true` — поворачиваться кратчайшим путём (не через весь круг). Радианы: полный круг = `2 * .pi`, половина = `.pi`, 90° = `.pi / 2`.

#### Проверка нажатия на кнопку — nodes(at:) + имя родителя
...
`node.name ?? node.parent?.name` — проверяем имя самого узла, **а если у него нет имени — имя родителя**. Нужно, потому что нажать можно и на кнопку, и на текст внутри неё (у текста имени нет, но его родитель — кнопка).

#### Скорость пули — velocity

Пулю не двигают действием — ей задают скорость, и физика несёт её сама по прямой.

```swift
let length = sqrt(dx * dx + dy * dy)
guard length > 0 else { return }               // защита от деления на ноль
let speed: CGFloat = 700
// нормализуем направление (длина 1) и умножаем на скорость
bullet.physicsBody?.velocity = CGVector(dx: dx / length * speed, dy: dy / length * speed)
```

#### Волны и случайный интервал — wait(withRange:), repeat(count:)

```swift
let spawn = SKAction.run { self.spawnEnemy() }
let wait  = SKAction.wait(forDuration: 1.2, withRange: 0.6)   // пауза 1.2 ± 0.3 сек
// повторить РОВНО N раз (в отличие от repeatForever)
run(SKAction.repeat(SKAction.sequence([wait, spawn]), count: enemiesPerWave))
```

`withRange` даёт случайный разброс паузы — враги появляются «живо», а не механически ровно.

#### Перебрать все объекты по имени — enumerateChildNodes

```swift
// Например, убрать всех врагов при Game Over
...
func damageBase(_ amount: Int) {
    baseHealth -= amount
    combo = 0                        // пропустил врага — серия прервана
}
```

Комбо растёт только при попадании (`addScore`) и сбрасывается только при промахе/уроне (`damageBase`) — не перепутать, где что.

---

### Урок 18 — Звуки и частицы в игре

#### SKEmitterNode — частицы (в коде)

Частицы — это много маленьких объектов, которые «фабрика» создаёт сама: взрывы, огонь, дым, искры, салют. Настраиваешь одну фабрику `SKEmitterNode` — остальное SpriteKit делает сам. (Аналог `GPUParticles2D` в Godot.)

```swift
let burst = SKEmitterNode()
burst.particleTexture = dotTexture     // как выглядит одна частица (см. текстуру ниже)
burst.numParticlesToEmit = 120         // всего частиц (0 = бесконечно)
burst.particleBirthRate = 4000         // сколько рождается в секунду
burst.particleLifetime = 1.2           // сколько живёт каждая
burst.particleLifetimeRange = 0.5      // разброс времени жизни
burst.emissionAngle = 0
burst.emissionAngleRange = .pi * 2     // разброс направления (2π = во все стороны)
burst.particleSpeed = 180
burst.particleSpeedRange = 80
burst.yAcceleration = -120             // «гравитация» частиц (минус = вниз)
burst.particleScale = 0.4
burst.particleScaleSpeed = -0.3        // уменьшаются со временем
burst.particleAlpha = 1.0
burst.particleAlphaSpeed = -0.8        // гаснут
burst.particleColor = color
burst.particleColorBlendFactor = 1.0
burst.particleColorSequence = nil      // ВАЖНО! иначе particleColor игнорируется
...
| `numParticlesToEmit` | всего частиц: `0` = поток без конца, число = разовый залп |
| `particleLifetime` (+`Range`) | сколько секунд живёт частица |
| `emissionAngle` / `emissionAngleRange` | направление и разброс (в радианах) |
| `particleSpeed` (+`Range`) | скорость вылета |
| `particleScale` / `particleScaleSpeed` | размер и как он меняется (минус = уменьшается) |
| `particleAlpha` / `particleAlphaSpeed` | прозрачность и затухание |
| `yAcceleration` / `xAcceleration` | ускорение: гравитация вниз / ветер вбок |
| `particleColor` | цвет (обязательно с `particleColorSequence = nil`) |

#### Ловушка №1: particleColorSequence

Пока задана «цветовая дорожка» (`particleColorSequence`), свойство `particleColor` **игнорируется** — частицы получаются блёклыми. Задаёшь цвет вручную — обнуляй дорожку:

```swift
burst.particleColor = .red
burst.particleColorBlendFactor = 1.0
burst.particleColorSequence = nil   // теперь частицы будут красными
```

#### Залп или поток — numParticlesToEmit

Одно свойство решает, какой это эффект:

```swift
burst.numParticlesToEmit = 120   // ВЗРЫВ: ровно 120 частиц и стоп (разовый залп)
trail.numParticlesToEmit = 0     // ОГОНЬ / ДЫМ / ХВОСТ: сыплет бесконечно
```

#### targetNode — хвост «остаётся в воздухе»

Если фабрику частиц повесить на движущийся объект (`rocket.addChild(trail)`), уже вылетевшие частицы «прилипнут» и поедут вместе с ним. Чтобы рождённая частица оставалась на месте в мире:

```swift
trail.targetNode = self   // частицы живут в сцене, а не внутри ракеты — получается настоящий след
```

#### Своя текстура частицы (кружок вместо квадрата)

По умолчанию частицы — блёклые квадратики. Нарисуем светящийся кружок и превратим его в текстуру:
...
#### Частицы через редактор .sks (визуальный способ)

Можно не кодом, а мышкой (как в Godot): `File → New → File → SpriteKit Particle File`, выбрать шаблон (`Fire`, `Smoke`, `Spark`, `Rain`, `Snow`), настроить ползунками. Загрузка:
...
Ползунки редактора — это те же свойства кода:

| Параметр в редакторе | Свойство в коде |
|---|---|
| Birthrate | `particleBirthRate` |
| Lifetime | `particleLifetime` |
| Speed | `particleSpeed` |
| Emission → Angle / Range | `emissionAngle` / `emissionAngleRange` |
| Scale | `particleScale` |
| Color Blend | `particleColorBlendFactor` |
| Alpha | `particleAlpha` |

#### Короткий звук — playSoundFileNamed

```swift
run(SKAction.playSoundFileNamed("boom.wav", waitForCompletion: false))
```

`waitForCompletion: false` — «играй и не жди конца звука». Файл должен быть **добавлен в проект**: перетащить в список файлов Xcode, поставить галочки «Copy items if needed» и «Add to targets».

**Чтобы игра не падала без файла** — сначала проверь его наличие:
...
> `UserDefaults` хранит значение **на диске телефона** — оно переживает закрытие приложения (обычная переменная так не умеет). `integer(forKey:)` вернёт `0`, если ключа ещё нет. Ключ (`"highScore"`) должен быть **одинаковым** при записи и чтении — удобно вынести его в константу, чтобы не опечататься.
...
> **Нюанс: сцены не делят переменные.** При `presentScene` старая сцена убирается с холста и удаляется, а новая создаётся «с нуля» — её переменные пустые. Поэтому меню не может прочитать счёт из игры напрямую: рекорд передают через общее хранилище `UserDefaults` (игра записала → меню прочитало). Чтение рекорда ставь в `didMove` меню, чтобы после игры оно было свежим.
...
#### «Камера» через узел-контейнер

Чтобы «прокручивать» большой уровень (растущая башня, длинная карта), кладут объекты в **один узел-контейнер** и двигают его целиком — все дети едут вместе.

```swift
let world = SKNode()      // контейнер для игровых объектов
addChild(world)
world.addChild(block)     // объекты кладём ВНУТРЬ него

// «Камера» едет: двигаем контейнер, а не каждый объект
world.run(.moveTo(y: newY, duration: 0.15))
```

- Координаты детей — относительно контейнера (как child относительно parent).
- Фон и HUD добавляй **не** в контейнер (в саму сцену), чтобы они стояли на месте.

---

### Урок 20 — Камера, параллакс, наклон, игровой цикл

#### CoreMotion — управление наклоном телефона
...
func startTilt() {
    if motion.isDeviceMotionAvailable {          // на iPhone — да, в симуляторе — НЕТ
...
if let g = motion.deviceMotion?.gravity {
    sub.position.x += CGFloat(g.x) * 420 * dt    // -1 (влево) … +1 (вправо)
}
// когда сцена уходит:
override func willMove(from view: SKView) { motion.stopDeviceMotionUpdates() }
```

> `gravity.x` — направление силы тяжести по горизонтали (−1…+1) = наклон телефона. ⚠️ В **симуляторе датчиков нет** (`deviceMotion == nil`) — тестировать на настоящем iPhone; полезно оставить запасное управление пальцем. Если ось «наоборот» — поменяй знак. Обязательно `stopDeviceMotionUpdates()` в `willMove`, иначе датчик тратит батарею.

#### SKCameraNode — настоящая камера

```swift
let cam = SKCameraNode()
camera = cam       // сцена: показывай мир через эту камеру
addChild(cam)      // камеру тоже добавляем в дерево
cam.position = CGPoint(x: size.width / 2, y: size.height / 2)

// следовать за героем (в update). Батискаф погружается ВНИЗ, камера ниже него:
cam.position.y = sub.position.y - size.height * 0.20   // «+» если герой летит вверх
```

> То, что в точке `cam.position`, оказывается **в центре экрана**. Двигаешь камеру — «кадр» едет по большому миру. Прямой аналог `Camera2D` из Godot. Нужны **обе** строки: `camera = cam` (назначить) и `addChild(cam)` (в дерево).

#### HUD и кнопки — дети камеры

```swift
cam.addChild(scoreLabel)   // всегда на экране, не уезжает
// позиция — ОТ ЦЕНТРА экрана (0,0):
scoreLabel.position = CGPoint(x: -size.width/2 + 20, y: size.height/2 - 50)  // левый-верх
```

> Что добавлено в сцену — живёт в мире (камера уедет). Что добавлено в камеру — «приклеено» к экрану. Экран конца/кнопки тоже в камере; и касание по ним ищем в координатах камеры: `touch.location(in: cam)` + `cam.nodes(at:)`.

#### update(_:) и dt — движение каждый кадр

```swift
override func update(_ currentTime: TimeInterval) {
    if lastTime == 0 { lastTime = currentTime }
    var dt = CGFloat(currentTime - lastTime)   // прошло секунд
    lastTime = currentTime
    if dt > 0.05 { dt = 0.05 }                  // защита от рывка (пауза/лаг)
    sub.position.y -= diveSpeed * dt            // погружаемся: вниз = y уменьшается
}
```

> `update` вызывается ~60 раз/сек (аналог `_process(delta)` в Godot). Всегда умножай скорость на `dt` — тогда игра идёт одинаково на быстром и медленном телефоне. `lastTime` — свойство сцены (`var lastTime: TimeInterval = 0`).

#### Параллакс — глубина из слоёв

```swift
// фон-точки (звёзды/пузырьки) — дети камеры; каждый кадр сдвигаем на delta * factor
// ПРОТИВ движения героя (герой вверх → фон вниз; герой вниз → фон вверх):
s.position.y -= delta * layer.factor           // factor 0.35 = далеко/медленно, 0.8 = близко/быстро
if s.position.y < -half {                       // ушёл за край —
    s.position.y = half                          // вернуть с другого края (переиспользуем узел)
    s.position.x = CGFloat.random(in: -size.width/2...size.width/2)
}
```

> Дальние слои двигаются медленнее — создаётся ощущение объёма (как столбы и лес из окна машины). Одни и те же точки «переезжают» по кругу — не плодим новые. Знак сдвига зависит от направления движения героя.

#### Сбор/касание по расстоянию (без физики)

```swift
let dx = p.position.x - sub.position.x
let dy = p.position.y - sub.position.y
if hypot(dx, dy) < 12 + subRadius {   // расстояние < суммы радиусов → коснулись
    oxygen = min(100, oxygen + 18)     // жемчуг даёт кислород
    bonus += 25                         // и очки
    p.removeFromParent()
}
```

> `hypot(dx, dy)` = √(dx²+dy²) — расстояние между центрами. Радиус круга можно взять из рамки: `node.frame.size.width / 2`. Лёгкая замена физике (урок 15), когда нужен только факт касания — годится и для сбора (жемчуг), и для столкновений.

#### Полоска (кислород/здоровье) — сжимаем по X

```swift
bar.anchorPoint = CGPoint(x: 0, y: 0.5)   // якорь у ЛЕВОГО края
bar.xScale = value / maxValue              // 100% → 1.0, 50% → 0.5
bar.color = value < 30 ? .red : .green     // краснеет при малом запасе
```

> Якорь `(0, 0.5)` — полоска убывает вправо от левого края, а не из центра. Полоску (и её тусклый фон) добавляем в камеру — стоит на экране.

#### Спавн впереди и уборка позади

```swift
// Погружаемся вниз (y уменьшается), поэтому «впереди» = МЕНЬШЕ y.
// спавн, пока «граница» выше низа видимой зоны:
while nextY > cam.position.y - size.height {
    spawn(atY: nextY); nextY -= CGFloat.random(in: 160...260)
}
// уборка ушедших за ВЕРХ (позади нас; иначе копятся и тормозят):
let limit = cam.position.y + size.height
items.removeAll { if $0.position.y > limit { $0.removeFromParent(); return true }; return false }
```
...
### Урок 29 — SwiftUI: списки и данные
...
### Урок 30 — SwiftUI: загрузка данных из сети
...
| Краш при звуке | SpriteKit | `playSoundFileNamed` с отсутствующим файлом | Проверять `Bundle.main.url(...)` перед проигрыванием |
| Частицы блёклые, не того цвета | SpriteKit | Не обнулили `particleColorSequence` | `emitter.particleColorSequence = nil` |
| Хвост частиц «прилип» к объекту | SpriteKit | Нет `targetNode` | `emitter.targetNode = self` |
| Взрыв сыплет без конца | SpriteKit | `numParticlesToEmit = 0` у залпа | Задать число (напр. 120) |
| «Invalid redeclaration of 'rgb'» | Несколько сцен | `extension SKColor.rgb` объявлено в двух файлах | Объявить общие помощники один раз |
| Рекорд всегда 0 | UserDefaults | Разный ключ при записи и чтении | Один ключ-константа везде (`bestScoreKey`) |
