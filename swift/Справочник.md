...
### Урок 20 — Камера, параллакс, игровой цикл
...
```swift
// звёзды — дети камеры; каждый кадр сдвигаем вниз на delta * factor:
s.position.y -= delta * layer.factor           // factor 0.35 = далеко/медленно, 0.8 = близко/быстро
if s.position.y < -half {                       // ушла за низ —
    s.position.y = half                          // вернуть наверх (переиспользуем узел)
    s.position.x = CGFloat.random(in: -size.width/2...size.width/2)
}
```

> Дальние слои двигаются медленнее — создаётся ощущение объёма (как столбы и лес из окна машины). Одни и те же звёзды «переезжают» по кругу — не плодим новые.
