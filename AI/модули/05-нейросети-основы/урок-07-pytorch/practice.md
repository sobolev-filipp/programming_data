...
>
> **Зачем она нужна.** Проверить, что PyTorch-сеть учится так же, как наша ручная (5.6) — хотя backward здесь делает одна строка `loss.backward()`.
>
> **Что на осях.** X — номер эпохи. Y — потеря. Линия — ход обучения PyTorch-сети.
>
> **Что мы видим.** Знакомую **падающую** кривую, выходящую на дно — точно такую же по форме, как у нашей самодельной сети из прошлого урока.
>
> **Какой вывод.** Одинаковая форма кривой = PyTorch делает ровно то же обучение (forward → loss → backward → update), просто пишется короче и считается быстрее. Итоговая точность ~0.956 даже чуть выше нашей ручной — но принцип тот же. Кривая потерь — универсальный индикатор, он одинаков хоть на NumPy, хоть на PyTorch.
