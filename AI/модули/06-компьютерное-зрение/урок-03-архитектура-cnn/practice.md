...
> 🆓 **Бесплатный GPU есть, но ограничен:** в часы пик Colab может не дать GPU или перевести на CPU — это нормально. MNIST маленький, CNN обучится и на CPU за несколько минут. Если совсем медленно — оставь `range(2)` эпохи.
...
> Обучение занимает пару минут. Используй это время:
> - **🔮 Предскажи:** побьёт ли CNN обычную сеть (MLP) из урока 5.8 (~97%)? На сколько, по-твоему?
> - **👀 Наблюдай:** точность по эпохам (≈0.976 → 0.983 → 0.987). Уже после 1-й эпохи — выше, чем у MLP!
> - **✏️ Микро-задача:** посчитай руками размеры картинки: 28 →(пулинг)→ 14 →(пулинг)→ 7. Сколько каналов после `conv2`? *(ответ: 32)*
> - **💬 Обсуди:** почему свёртки «видят» картинку лучше, чем обычная сеть? *(ловят узор в любом месте, сохраняют соседство пикселей)*
...
>
> **Зачем она нужна.** Проверить работу свёрточной сети на конкретных цифрах, а не только по общей точности.
>
> **Что на картинке.** Сетка 2×5 тестовых цифр. Над каждой — ответ CNN. Зелёный заголовок = угадала, красный = ошиблась.
>
> **Что мы видим.** 10 цифр, почти все заголовки зелёные.
>
> **Какой вывод.** Много зелёного = CNN работает (подтверждает ~98–99% точности). Свёрточная сеть распознаёт цифры даже точнее обычной из Модуля 5, потому что «видит» их через края и узоры, а не по отдельным пикселям.
