...
> ### 📊 Как читать эту сетку картинок (виды аугментации)
>
> **Зачем она нужна.** Показать, что делает каждый вид аугментации. Пять картинок из одной наглядно объясняют, как из малого числа фото получить много.
>
> **Что на картинке.** 5 картинок в ряд: оригинал и 4 преобразования (отражение, поворот, обрезка, изменение яркости). Все — из ОДНОЙ исходной картинки.
>
> **Что мы видим.** Один и тот же объект, но по-разному: зеркальный, наклонённый, обрезанный, посветлее/потемнее.
>
> **Какой вывод.** Все варианты — по-прежнему тот же класс (кот остаётся котом), но для сети это «новые» картинки. Так из 50 000 фото получаются «миллионы». Важно: аугментации должны сохранять класс (цифру 6 нельзя переворачивать — станет 9), поэтому подбирают их с умом.
...
> ### 📊 Как читать эту сетку картинок (случайные варианты)
>
> **Зачем она нужна.** Показать ключевую деталь: аугментация **случайна**. Один и тот же код, применённый 5 раз, даёт 5 разных картинок.
>
> **Что на картинке.** 5 картинок — это одна и та же исходная, но пропущенная через случайную аугментацию 5 раз подряд.
>
> **Что мы видим.** Пять непохожих вариантов: разный поворот, отражение, обрезка — хотя код и картинка те же.
>
> **Какой вывод.** Каждый прогон = новая случайная комбинация преобразований. Поэтому в каждую эпоху обучения сеть видит **свежие** варианты и не может тупо зазубрить конкретные пиксели — ей приходится учить суть объекта. Так аугментация борется с переобучением.
