...
> 🆓 **Бесплатный GPU есть, но ограничен:** в часы пик Colab может не дать GPU — это нормально. На CPU обучение долгое.
> 🐢 **CPU-fallback (если GPU нет):** возьми меньше эпох — `range(8)` вместо 15 — и/или подвыборку (`Subset`). Прирост от аугментации будет скромнее (она раскрывается на длинном обучении), но эффект «меньше переобучения» всё равно видно.
...
> Обучение с аугментацией идёт 15 эпох — это небыстро. Используй паузу:
> - **🔮 Предскажи:** поднимет ли аугментация точность по сравнению с 6.4 (~70%)? Станет ли разрыв train–test **меньше**?
> - **👀 Наблюдай:** аугментация раскрывается на **длинном** обучении — следи, продолжает ли точность расти к 15-й эпохе.
> - **✏️ Микро-задача:** нарисуй кота и его 4 аугментации (отражение, поворот, обрезка, яркость). Все ещё «кот»? А что станет с цифрой **6** при перевороте на 180°?
> - **💬 Обсуди:** какие аугментации **опасны** для цифр и почему? *(переворот/отражение: 6↔9)*
