...
> ### 📊 Как читать график границы решений
>
> **Зачем он нужен.** Показать, КАК модель делит пространство на классы. Обычно решение модели — это чёрный ящик, а тут мы буквально видим «карту»: в какую зону попадёт любая новая точка.
>
> **Что на осях.** X и Y — два признака цветка (длина и ширина лепестка, стандартизованные). Каждая точка с обводкой — реальный цветок, её цвет = настоящий вид. **Фон** раскрашен в бледные зоны (`contourf`): это территории, которые модель отдала каждому классу. Мы закрасили фон, спросив у SVM ответ для тысяч точек-«пикселей» сетки.
>
> **Что мы видим.** Три цветные области, разделённые плавными кривыми линиями. Точки-цветы лежат каждая на фоне своего цвета.
>
> **Какой вывод.** Любая новая точка получит класс той зоны, куда попадёт. Плавные (не прямые) границы — заслуга RBF-ядра, оно умеет гнуть разделяющую линию. Если точка одного цвета оказалась на чужом фоне — это ошибка модели. Здесь почти все точки «в своей зоне» → SVM разделил виды хорошо.
