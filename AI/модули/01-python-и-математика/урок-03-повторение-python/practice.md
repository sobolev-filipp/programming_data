...
> `key=lambda m: m[1]` читается как «сравнивай фильмы по второму элементу (рейтингу)». Обрати внимание: в `key` мы **передаём саму функцию-правило** (без скобок, без вызова) — `sort` сам применит её к каждому фильму по очереди. Поэтому пишем `key=lambda ...` (правило), а не готовое значение вроде `key=movies[1]`.
